...
class Cliente(self):
  def __init__(self, nome, sexo, renda):
    self.nome = nome
    self.sexo = sexo

class Conta_Corrente(Cliente):
  def __init__(self, nome, sexo, renda):
    super__init__(nome, renda, sexo)
    self.saldo = 0 

    def saque(self, valor):
      self.saldo -= valor
      if self.sexo == "F":
        if self.valor < -self.renda
          return (self.saldo += valor)
          print('Este valor de saque não é permitido.')
        else:
          return self.saldo
      else:
        if self.valor < 0
